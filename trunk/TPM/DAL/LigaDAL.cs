...
namespace TPM.DAL
{
    public class LigaDAL
    {
            public DataTable LigaGetAll()
            {
                ...
                {
                    using (SqlCommand cmd = new SqlCommand("LigaGetAll", con))
                    {
            ...
            public DataTable LigaById(int ligaId)
            {
                var dt = new DataTable();
                SqlDataReader sqlDataReader;
                ...
                using (SqlConnection con = new SqlConnection(HelperDal.GetConnection()))
                {
                    using (SqlCommand cmd = new SqlCommand("LigaById", con))
                    {

                        cmd.CommandType = CommandType.StoredProcedure;

                        cmd.Parameters.Add("@Id", SqlDbType.VarChar).Value = ligaId;
                        //cmd.Parameters.Add("@LastName", SqlDbType.VarChar).Value = txtLastName.Text;
